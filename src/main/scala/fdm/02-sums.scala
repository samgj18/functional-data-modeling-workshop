...
object enum_basics {
  ...
  // sealed keyword are the Scala 2.x mechanism to create Scala Sum types. The keyword sealed means that
  // all subtypes of ChessPieceType must be defined within the same file.
  ...
  object Currency {
    ...
    final class INR(rupees: Int, paise: Int) extends Currency
  }
...
object enum_utilities {
   ...
   */
  card match {
    case Card.Clubs(points) => println(s"Clubs: $points")
    case _                => println("Did not match!")
  }
   ...
   */
  card match {
    case Card.Spades(10) => println("Spades: 10")
    case _               => println("Did not match!")
  }
   ...
   */
  card match {
    case Card.Spades(points) if points >= 10 => println(s"Spades: $points")
    case _                                  => println("Did not match!")
  }
   ...
   */
  card match {
    case spades @ Card.Spades(_) => println(s"Spades: $spades")
    case _                      => println("Did not match!")
  }
   ...
   */
  card match {
    case Card.Spades(_) | Card.Diamonds(_) => println("Spades or Diamonds")
    case _                                => println("Did not match!")
  }
}
...
object enum_generics {
   ...
   * cases of the enum.
   */
  sealed trait AdvertisingEvent[+A]
  object AdvertisingEvent {
    case object None                                                           extends AdvertisingEvent[Nothing]
    final case class Impression[A](pageUrl: String, data: A)                 extends AdvertisingEvent[A]
    final case class Click[A](pageUrl: String, elementId: String, data: A)   extends AdvertisingEvent[A]
    final case class Action[A](pageUrl: String, actionName: String, data: A) extends AdvertisingEvent[A]
  }
   ...
   */
  lazy val advertisingEvent = AdvertisingEvent.Click("/home", "42", 12)
